...
# Pode instalar mais de um plugin separando por espaço (ex.: "postgresql commandlinemetrics")
...
run_liquibase() {
  ...
  # 1) Instala o(s) plugin(s) necessário(s) na cache persistida (idempotente)
  ...
  # 2) Executa o comando com o plugin já disponível
  docker run --rm --network host -w /workspace \
